...
###########################################################################################################
    ...
    def kernel_header_devel(self):
        ...
        self.sut.execute_shell_cmd('unzip *.zip', timeout=5*60, cwd=KERNEL_DEVEL_PATH_L)
    ...
    def qat_install(self, is_siov=False):
        ...
        print("==============================")
        print("Mona QAT_Driver_H", QAT_DRIVER_H)
        print("Mona QAT_Driver_Path_L",QAT_DRIVER_PATH_L)
        print("============================= ")
        ...
        print("==============================")
        print("Mona CP2 _:", _)
        print("Mona CP2 OUT:",out)
        print("Mona CP2 ERR:", err)
        print("============================= ")
        #if "Unit qat_service.service could not be found" in str(err):
        if "Unit qat_service.service could not be found" in str(out):
    ...
    def qat_service_restart(self):
        ...
        # Different versions of QAT have differing output signatures. Code below permits both versions.
        try:
    ...
    def qat_uninstall(self):
        ...
        _, out, err = self.sut.execute_shell_cmd('service qat_service status', timeout=60)
        if not "Unit qat_service.service could not be found." in str(err):
        #if out != '':
            #if out != '0\n':
                # self.sut.execute_shell_cmd('reboot', timeout=30*60)
                # self.Case.sleep(60)
            ...
            # if not "Unit qat_service.service could not be found" in str(err):
            print("=============================")
            print("mona cp1-1 _",_)
            print("mona cp1-2 out",out)
            print("mona cp1-3 err",err)
            print("=============================")
            if not "Unit qat_service.service could not be found." in str(err):
    ...
    def qatzip_test(self):  #qatzip_script-->qatzip.sh
        ...
        self.sut.execute_shell_cmd('./autogen.sh', timeout=5 * 60, cwd=f'{QATZIP_PATH_L}QATzip-master/')
    ...
    def rpm_install(self):
        ...
        for rpm in rpm_list:
            #self.sut.execute_shell_cmd(f'yum -y install {rpm}', timeout=60000) Mona 20230208
            self.sut.execute_shell_cmd(f'dnf -y install {rpm}', timeout=60000)
    ...
    def run_dpdk(self):
        ...
        self.sut.execute_shell_cmd('yum install -y meson python3-pyelftools', timeout=10*60)
...
###############################################################################################
    ...
    def kernel_header_devel_vm(self, qemu, vm_name):
        ...
        print("=====================================================")
        print("kernelPath",KERNEL_DEVEL_PATH_L)
        print("=====================================================")
        qemu.upload_to_vm(vm_name, sut_file_dir1, vm_file_dir1)
        qemu.execute_vm_cmd(vm_name, 'unzip *.zip', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L) # Mona 
        qemu.execute_vm_cmd(vm_name, 'rpm -ivh *.rpm --force --nodeps', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L)
        #qemu.execute_vm_cmd(vm_name, 'dnf install *.rpm', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L)
    ...
    def kernel_header_internal_devel_rich_vm(self, qemu):
        ...
        qemu.execute_rich_vm_cmd_parallel('unzip *.zip', timeout=10 * 60,
                                          cwd=KERNEL_INTERNAL_PATH_L) # Mona
        qemu.execute_rich_vm_cmd_parallel('rpm -ivh *.rpm --force --nodeps', timeout=10 * 60,cwd=KERNEL_INTERNAL_PATH_L)
        #qemu.execute_rich_vm_cmd_parallel('dnf install *.rpm', timeout=10 * 60,
    ...
    def kvm_kernel_header_devel_vm(self, kvm, vm_name):
        ...
        kvm.execute_vm_cmd(vm_name, 'unzip *.zip', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L) # Mona
        kvm.execute_vm_cmd(vm_name, 'rpm -ivh *.rpm --force --nodeps', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L)
        #kvm.execute_vm_cmd(vm_name, 'dnf install *.rpm', timeout=5 * 60, cwd=KERNEL_DEVEL_PATH_L)      
    ...
    def qat_install_vm(self, qemu, vm_name, execute_command, enable_siov=False):
        ...
        """
        logger.info("========================================================")
        logger.info("Mona CP1 Self",self)
        logger.info("Mona CP1 VM_name",vm_name)
        logger.info("Mona CP1 Execute_command",execute_command)
        logger.info("========================================================")
        qemu.execute_vm_cmd(vm_name, f'mkdir -p {QAT_DRIVER_PATH_L}', timeout=60)
        logger.info("Mona CP1 QAT_Driver_Path",QAT_DRIVER_PATH_L)
    ...
    def run_dpdk_vm(self, qemu, vm_name):
        ...
        qemu.execute_vm_cmd(vm_name, 'yum install -y meson python3-pyelftools', timeout=10 * 60)
...
#############################################################################################################
    ...
    def __check_error(self, err):
        # Ignore CryptographyDeprecationWarning messages from paramiko
